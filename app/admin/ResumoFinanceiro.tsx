...
// Renamed to avoid collision with the component function
interface DadosResumoFinanceiro {
...
// Animated bar for monthly chart
...
export default function ResumoFinanceiro() {
  ...
  const [dados, setDados] = useState<DadosResumoFinanceiro | null>(null);
  ...
  const calcularResumo = async () => {
    try {
      ...
      if (usuario?.tipo === 'cliente') {
        ...
        for (let i = 0; i < 7; i++) {
          try {
            ...
            const agendamentosDoDia = await api.getAgendamentosPorData(dataStr);
            todosAgendamentos.push(...agendamentosDoDia);
          } catch (error) {
            // silently skip days with no appointments
          }
        }
      ...
      const ticketMedio = totalServicosConcluidos > 0
        ? receitaTotal / totalServicosConcluidos
        : 0;

      const taxaConclusao = totalAgendamentos > 0
        ? (totalServicosConcluidos / totalAgendamentos) * 100
        : 0;
      ...
      agendamentosConcluidos.forEach(agendamento => {
        try {
          ...
          console.log('Erro ao processar data:', agendamento.dataAgendamento);
        }
      });
      ...
      const servicosPorMes = Array.from(servicosPorMesMap.values())
        .sort((a, b) => {
          ...
          // Fixed: added .getTime() to avoid type error on Date arithmetic
      ...
      setDados({
        ...
        receitaTotal,
        ticketMedio,
        servicosPorMes: servicosPorMes.slice(0, 6),
        servicoMaisPopular,
        servicoMaisPopularCount: maxCount,
        taxaConclusao,
      });
  ...
  const colors = {
    ...
    gradientStart: isDark ? '#1E293B' : '#EEF2FF',
    gradientEnd: isDark ? '#0F172A' : '#F8FAFC',
      ...
      <View style={[estilos.loadingContainer, { backgroundColor: colors.bg }]}>
        <View style={[estilos.errorCard, { backgroundColor: colors.cardBg }]}>
          ...
          >
            ...
            <Text style={[estilos.retryText, { fontFamily: 'Poppins_600SemiBold' }]}>
              Tentar Novamente
            </Text>
          </TouchableOpacity>
        </View>
      ...
      >
        <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
          ...
          >
            ...
            <Text style={[estilos.heroLabel, { fontFamily: 'Poppins_400Regular' }]}>
              Receita Total
            </Text>
            <AnimatedNumber
            ...
            <View style={estilos.heroFooter}>
              <View style={estilos.heroFooterItem}>
                <Text style={[estilos.heroFooterLabel, { fontFamily: 'Poppins_400Regular' }]}>
                  Ticket Médio
                </Text>
          ...
          <View style={[estilos.chartCard, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
            ...
            {dados?.servicosPorMes && dados.servicosPorMes.length > 0 ? (
              <>
                ...
                {dados.servicosPorMes.map((mes, index) => (
                  <View key={index} style={[estilos.monthItem, index < dados.servicosPorMes.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}>
              ...
              <View style={estilos.emptyState}>
                ...
                <Text style={[estilos.emptyText, { color: colors.textSec, fontFamily: 'Poppins_400Regular' }]}>
                  Nenhum serviço concluído nos últimos meses
                </Text>
              </View>
            )}
          </View>
